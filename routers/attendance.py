...
def mark_attendance(attendance: AttendanceSchema, current_user_data: tuple = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if subject.teacher_id != current_user.id:
        logger.warning(f"Teacher {current_user.email} attempted to mark attendance for subject {attendance.subject_id} they don't own")
    ...
    if student.faculty != subject.faculty:
        logger.warning(f"Student {attendance.student_id} faculty {student.faculty} doesn't match subject {attendance.subject_id} faculty {subject.faculty}")
    ...
    if existing_attendance:
        logger.warning(f"Duplicate attendance - student {attendance.student_id} already marked for subject {attendance.subject_id} on {attendance.date}")
